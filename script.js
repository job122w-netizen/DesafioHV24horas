...
function drawClock(){
  ...
  [...activities].sort((a,b)=>timeToMin(a.start)-timeToMin(b.start)).forEach(act=>{
    ...
    const opacity=act.done?0.22:isSkipped?0.15:isExpired?0.35:0.88;
    html+=`<path d="M${x1.toFixed(2)},${y1.toFixed(2)} A${arcR},${arcR} 0 ${large},1 ${x2.toFixed(2)},${y2.toFixed(2)} L${xi1.toFixed(2)},${yi1.toFixed(2)} A${innerR},${innerR} 0 ${large},0 ${xi2.toFixed(2)},${yi2.toFixed(2)} Z" fill="${act.color}" opacity="${opacity}" stroke="rgba(7,7,26,0.4)" stroke-width="0.8"/>`;
    if(act.done){const midA=(sA+eA)/2+(large?Math.PI:0),midR=(arcR+innerR)/2,mx=cx+midR*Math.cos(midA),my=cy+midR*Math.sin(midA);html+=`<text x="${mx.toFixed(2)}" y="${my.toFixed(2)}" text-anchor="middle" dominant-baseline="central" font-size="7" fill="${act.color}" opacity="0.9">✓</text>`;}
  });
...
function cardHTML(act,badge,isNow,isNext,canCheck,isDone,isSkipped,isExpired){
  return`<div class="act-card ${isDone?'completed':''} ${isNow?'active-now':''} ${isNext?'next-up':''} ${!canCheck&&!isDone&&!isSkipped?'locked':''} ${isSkipped?'skipped-act':''} ${isExpired?'expired-act':''}" id="card-${act.id}" style="--act-color:${act.color}">
    ...
    <div class="act-body"><div class="act-name ${isSkipped?'struck':''}${isExpired?' expired-name':''}">${act.name}${badge}</div><div class="act-time">${act.start} — ${act.end}</div></div>
    <div class="act-right">
...
function toggleAct(id){
  ...
  }else{const cur=getTotalStars();if(cur>0)localStorage.setItem('dhv_total_stars',cur-1);}
  saveData();render();
}
